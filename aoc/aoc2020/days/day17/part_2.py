...
def parse(initial_state: str) -> Pocket:
    pocket = defaultdict(lambda: defaultdict(lambda: defaultdict(
...
def find_active_points(pocket: Pocket) -> Coords:
    return [(x, y, z, w)
            for w, universe in pocket.items()
            for z, plane in universe.items()
...
def change_states(pocket: Pocket, active_points: Coords,
    ...
    for (x, y, z, w), count in active_neighbor_count.items():
        stay_active = pocket[w][z][y][x] and count in [2, 3]
        become_active = not pocket[w][z][y][x] and count == 3
        new_pocket[w][z][y][x] = stay_active or become_active
    return new_pocket


def solve_part_two(initial_state: str) -> int:
...
if __name__ == "__main__":
    ...
    result = solve_part_two(initial_state)
    print(result)
